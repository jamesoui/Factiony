  ...
  useEffect(() => {
    ...
    fetch(`${supabaseUrl}/functions/v1/get-discovery-catalog?limit=15`, {
      ...
      .then((data) => {
        ...
        console.log(
          "[DISCOVER] DISCOVERY_TOP_RATED_COUNT:",
        ...
        if (data.ok && data.topRated && data.topRated.length > 0) {
          ...
          console.warn(
            "[DISCOVER] No games from Edge Function, using legacy fallback..."
          );
          fetch(
            `${
              import.meta.env.VITE_API_URL ??
              "https://europe-west1-factiony-1fc0f.cloudfunctions.net/apiFunction"
            }/games?ordering=-metacritic&metacritic=80,100&page_size=15`,
            {
              headers: {
                "x-factiony-key":
            ...
            .then((r) => {
              console.log(
                "[DISCOVER] Legacy fallback returned:",
                r.results?.length || 0,
                "games"
              );
              const formattedGames =
                r.results?.map((game: any) => ({
                  ...game,
        ...
        if (res.results?.length) {
          ...
          fetch(
            `${
              import.meta.env.VITE_API_URL ??
              "https://europe-west1-factiony-1fc0f.cloudfunctions.net/apiFunction"
            }/games?page_size=10&page=${randomPage}`,
            {
              headers: {
                "x-factiony-key":
                  import.meta.env.VITE_FACTIONY_KEY ??
        ...
        {topRated === null ? (
          ...
          <div className="text-center py-12 bg-gray-800 rounded-lg">
            ...
            <p className="text-sm text-gray-400 mb-6">
              Explore des jeux pour alimenter le catalogue
            </p>
            <a
              href="/search"
